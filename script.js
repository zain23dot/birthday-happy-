...
function checkPassword() {
    ...
    if (input === CONFIG.password) {
        ...
        if (error) {
            ...
            error.textContent = "Incorrect passcode! Try again ❤️";
        }
...
function openFinalGift() {
    ...
    // Automatically reveal the ultimate cinematic panel right as the box pops!
    setTimeout(() => {
        revealUltimatePage();
    }, 1500);
}
...
function revealUltimatePage() {
    ...
    // Dense romantic heart particle burst explosion
    for (let i = 0; i < 40; i++) {
        setTimeout(() => {
            createHeartParticle();
        }, i * 45);
